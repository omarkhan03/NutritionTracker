...
public class Entry implements Comparator<Entry>{
    ...
    public String getSurplusDefficiency() {
        if(this.consumption > this.nutrient.getTarget()){ //determines if a surplus or deficiency is present on average, or is the user has meets their daily target on a given day
            ...
            return ("You were deficient in " + this.nutrient.getName() + " by " + (this.nutrient.getTarget() - this.consumption) + " units based on your target goal on day " + this.day);
        }
        return null;
    }
